...
protocol AccountNetworkProtocol {
    ...
    func register(param: [String : Any], completion: @escaping (LoginModel) -> (), failure: @escaping (String) -> ())
}
...
class AccountNetwork: AccountNetworkProtocol {
    ...
    func register(param: [String : Any], completion: @escaping (LoginModel) -> (), failure: @escaping (String) -> ()) {
        let url = AccountManager.register.path
        network.post(type: LoginModel.self, url: url, params: param) { response in
            switch response {
            case .success(let model):
                completion(model)
            case .failure(let error):
